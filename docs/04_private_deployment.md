...
- 本看板**仅供 Sera 向思源哥（Siyuan.C）展示 HTX OTC BD / PIP 进度**，是内部汇报工具，**不作为公开网站**。
- 最终交付形态：一个私密链接，**只有思源哥本人通过真实身份验证（邮箱一次性验证码 OTP / Google 登录）后可访问**。
...
| --- | --- | --- | --- |
| 适用条件 | 思源哥本人有 GitHub 账号；仓库所有者为付费计划（私有仓 Pages 需 GitHub Pro/Team 及以上）；将思源哥邀请为仓库 collaborator | 任意静态站；思源哥只需有一个邮箱（或 Google 账号）；Sera 注册 Cloudflare 账号（免费套餐即可） | 有一台 VPS，或使用 Render / Railway 等平台托管静态站 |
| 访问控制方式 | 登录身份 = 思源哥自己的 GitHub 账号，只有 collaborator 能看到私有仓 Pages | Cloudflare Access 在边缘拦截请求，邮箱一次性验证码（OTP）/ Google 登录，Policy 只放行思源哥邮箱 | HTTP Basic Auth，指定用户名密码（如用户名 `siyuan`，密码由 Sera 单独告知） |
...
| 限制 | 需付费计划才支持私有仓 Pages；要求思源哥注册并登录 GitHub；collaborator 可见整个仓库（含提交历史），权限粒度过粗 | 首次使用需在 Zero Trust 控制台配置，略有学习成本；使用 Pages 分配的 `*.pages.dev` 域名即可，无需自有域名 | 浏览器弹窗体验较粗糙；**密码必须放平台环境变量，严禁写进前端代码或 git**；自建 VPS 需自行维护 nginx、证书与续期 |
| 结论 | 备选（仅当思源哥本就是活跃 GitHub 用户时） | **推荐** | 备选（不使用 Cloudflare 时的兜底方案） |
...
3. **可只放行思源哥邮箱**：Policy 支持按邮箱地址精确 Include，名单里只放思源哥一个邮箱，其他人一律拒绝。
...
> 目标产出：一个 `https://<项目名>.pages.dev` 私密链接，仅思源哥邮箱可通过验证。
...
4. **创建 Policy**：在该应用下 Add a policy → **Include** 规则选择 **Emails**，值填**思源哥邮箱**（只此一个）→ Save。确认没有多余的 Allow 规则。
5. **开启登录方式**：Zero Trust → Settings → Authentication → **Login methods**，开启 **One-time PIN**（邮箱一次性验证码）；如思源哥习惯用 Google 账号，可叠加开启 **Google**。
6. **保存后测试**：
   - 用**无痕窗口**打开链接 → 应被 Cloudflare Access 拦截并要求验证，而不是直接看到看板；
   - 输入一个**不在名单内的邮箱** → 应收不到验证码 / 被直接拒绝；
   - 输入思源哥邮箱 → 收到验证码，验证通过后可正常打开看板（顺便检查 Day/Night 主题与数据加载正常）。
7. **交付链接**：把链接发给思源哥，并**单独告知**：打开链接后需要输入他自己邮箱收到的验证码才能进入（这同时证明链接没有发错人）。
...
2. 在平台后台配置**环境变量**（严禁写进代码或 git）：
   - `BASIC_AUTH_USER=siyuan`
   - `BASIC_AUTH_PASSWORD=<由 Sera 单独告知思源哥的密码>`
...
2. 生成密码文件：`htpasswd -c /etc/nginx/.htpasswd siyuan`（密码文件放 web 根目录之外）。
3. 站点配置中加入：
...
- [ ] 思源哥邮箱验证通过后可正常访问，Day/Night 主题、搜索、筛选、Export 均正常。
- [ ] 页面与仓库中**无真实客户信息**（无真实客户姓名、HTX UID、TG 用户名/路径、银行账户、邮箱、电话——同事名 Sera/思源哥/静格/Oscar 除外），客户均为 Client-001 等占位编号。
- [ ] 等待会话过期（或手动清除 Cookie）后重新打开链接，**需重新验证**才能进入。
- [ ] Access Policy 名单复核：仍只有思源哥一个邮箱。
