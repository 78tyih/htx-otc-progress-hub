...
> 适用范围：v0.3「PIP 进度管理 Dashboard」升级后的页面结构与数据文件
...
- 三条线各自的阻塞互不传染：例如「大数据名单」Blocked（待思源哥协助调取）不影响 Partner/KOL 物料制作与触达。
...
| WS6 | 私密看板线 | 看板从公网转为私密访问：仓库 PRIVATE、公网 Pages 下线，Cloudflare Pages + Access 仅放行思源哥 | **65%**（仓库已私有、方案与文档已备，待 Cloudflare 部署） |
...
3. 脱敏自查：无真实客户姓名、HTX UID、TG 用户名/路径、银行账户、邮箱、电话（同事名 Sera/思源哥/静格/Oscar 除外）
...
> 相关文档：私密部署操作清单见 `docs/04_private_deployment.md`；安全与脱敏规范见 `SECURITY.md`；版本变更见 `docs/CHANGELOG.md`。
