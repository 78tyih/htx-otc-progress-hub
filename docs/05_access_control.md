...
- **最小权限**：看板只服务一个目的——Sera 向思源哥汇报 OTC BD / PIP 进度。因此访问权限只授予完成这一目的所必需的最小范围。
- **仅思源哥一个邮箱**：Cloudflare Access Policy 的 Include 名单中**只放思源哥一个邮箱**，不添加任何备用邮箱、群组域名或泛域名规则。
- **任何人入名单需显式决策**：未来如需新增访问者，必须先由思源哥确认，再临时加入，用后及时移除，并在当周安全检查中复核。
...
| 项目 | 策略 |
| --- | --- |
| 会话时长 | 在 Zero Trust → Access → 对应 Application 的 **Session Duration** 中设置，建议 **12–24 小时**：思源哥每天看一两次，不需要频繁重验；过期后自动要求重新验证 |
...
3. **仓库内只放 `.env.example` 模板**：只含键名、占位值和注释说明（如 `ALLOWED_EMAIL=siyuan@example.com`），新人/未来的自己照模板在本地填真实值。
...
| 级别 | 内容 | 存放位置 | 说明 |
| --- | --- | --- | --- |
| ✅ 可入仓库（脱敏） | 汇总 KPI 数字与目标、Pipeline 状态 / 进度 / 下一步、设计交付清单、漏斗各级汇总数量、客户占位编号（Client-001 等）、同事名（Sera / 思源哥 / 静格 / Oscar） | GitHub private repo + 私密站点 | 只放统计结果与状态，不放到个人 |
...
4. **评估数据影响**：确认泄露窗口期内可能被看到的内容范围（仅脱敏汇总数据，还是混入了敏感信息）；若有敏感信息进过 git 历史，需改写历史或重建仓库，并按公司流程上报思源哥。
