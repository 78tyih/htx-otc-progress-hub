...
### 安全
- QA 独立终验 12 项全部 PASS、零修复：临时服务（8123 端口，避开用户 8080 预览进程）15 项资源全 200、进程无残留；7 个 JSON 合法且 FALLBACK 逐字一致；`node --check` 通过；全仓库（排除 .git）`思源|siyuan` 0 命中；敏感信息扫描（客户姓名/UID/TG/银行/邮箱/电话，含点文件与 SVG 共 34 个文本文件）0 命中；无头浏览器实测 1280 / 390 视口无横向溢出、console 零报错。
