...
- 页面改名「HTX OTC PIP 执行看板」，副标题「Sera · OTC USD 大宗业务绩效任务进展」，新增「审核对象：Simon」徽标
- 侧栏 9 项目录加状态点（已完成绿 / 进行中黄 / 阻塞红 / 待启动灰；阻塞项动态红绿）
- 甘特胶囊条四态重定义：完成绿 / 进行黄色渐变 / 待启动灰半透明 / 阻塞红边条；03 区改名「主线任务时间推进图」、04 区改名「任务依赖关系图」
- 依赖图支线 A 增加 Partner / KOL 第四来源节点
- 阻塞事项改双栏（新增 `data/blockers.json`：current 当前阻塞 / asks 需 Simon 协助），移除 FIXED_ASKS 与 pipeline 派生阻塞卡
- 周更记录轻量化（`weekly-log.json` 重构为 cadence/updatedAt/done[]）
- KPI 第 4/5 卡对齐绩效表（客户资料·手册 ring 1/1、客户 Pipeline 1/1）；主线进度卡渲染 subLabel；Pipeline 卡片两行收敛+卡高一致
- 执行摘要「当前阻塞」改由 blockers.json 驱动（4 项）；加载数据源 7→8，`sync_fallback.py` 同步升级
- 数据对齐：gantt L1=80 / L2=70、roadmap 五线绩效口径、todo 五条绩效口径
